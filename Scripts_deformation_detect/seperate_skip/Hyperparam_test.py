...
if __name__ == "__main__":
    ...
    """Loading"""
    good_u, bad_u = [], []
    difs, std_1s, std_2s, combos = [], [], [], []
    with open("good_u.dat", "rb") as f:
        good_u = np.array(pickle.load(f))
    with open("bad_u.dat", "rb") as f0:
        bad_u = np.array(pickle.load(f0))
    difs = good_u - bad_u
    ...
    comparisons = 3
    topn = 80  # 2: dif vs. std_sum | 3: dif vs. std_1 vs. std_2
    """
    ...
    if comparisons == 1:
        ...
        difs_topn = []
        for i in range(topn):
            difs_topn.append(np.where(difs == difs_sort[i])[0][0])
        intersects = difs_topn
        ...
        file2 = open("Comp_1_ways_top{}.txt".format(topn), "w")
        for j in intersects:
            std_1 = std_1s[j]
            ...
            result = "Combo {}: std_1: {:.2f} with rank: {} | std_2: {:.2f} with rank: {} | dif: {:.3f} with rank: {}" \
        ...
        plt.legend(['std_1s', 'std_2s', 'difs'], loc='upper left')
        ax1 = plt.gca()
        ax1.set_xlabel(r"Iterations")
        plt.savefig("Comp_1_ways_top{}.png".format(topn))
        plt.show()
        plt.close()
    ...
    if comparisons == 2:
        ...
        ax1 = plt.gca()
        ax1.set_xlabel(r"Iterations")
        plt.savefig("Comp_2_ways_top{}.png".format(topn))
        plt.show()
        plt.close()
    ...
    if comparisons == 3:
        ...
        """ Special Zero Edge Case """
        std_1s_zeros, std_2s_zeros = np.where(std_1s == 0)[0], np.where(std_2s == 0)[0]
        std_1s_topn, std_2s_topn = list(std_1s_zeros), list(std_2s_zeros)
        for i in range(topn):
            if std_1s_sort[i] != 0:
                std_1s_topn.append(np.where(std_1s == std_1s_sort[i])[0][0])
            if std_2s_sort[i] != 0:
                std_2s_topn.append(np.where(std_2s == std_2s_sort[i])[0][0])
            difs_topn.append(np.where(difs == difs_sort[i])[0][0])
        ...
        ax1 = plt.gca()
        ax1.set_xlabel(r"Iterations")
        plt.savefig("Comp_3_ways_top{}.png".format(topn))
        plt.show()
        plt.close()
