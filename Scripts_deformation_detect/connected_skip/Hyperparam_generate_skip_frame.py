...
def generate(skip, r_threshold, winsize):
    ...
    while video_fp.isOpened():
        ...
        if prev_frame is not None:
            ...
            #         state = "recording"
        ratios.append(ratio)
        # print("Temporary_frame_index is: {}".format(frame_index))
    ...
    good_u, good_std = np.mean(ratios[-5:]), np.std(ratios[-5:])
    bad_u, bad_std = np.mean(ratios[1:-6]), np.std(ratios[1:-6])
    ...
    plt.title(title)
    plt.tight_layout()
    plt.savefig(str(result_path / "skip_{}_thres{:.1f}_winsize_{}.png".format(skip, r_threshold, winsize)))
    # plt.show()
    plt.close()
    tq.close()
...
if __name__ == "__main__":
    ...
    for video_path in video_path_list:
        ...
        skips = np.arange(10, 81, 10).tolist()  # 10, 41, 5
        r_thresholds = np.arange(0.25, 1.26, 0.25).tolist()  # 0.25, 1.26, 0.25
        winsizes = np.arange(30, 81, 10).tolist()  # 10, 41, 5
